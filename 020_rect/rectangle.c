...
rectangle_t intersection(rectangle_t r1, rectangle_t r2) {
  ...
  // get
  int x_res = max(r1.x, r2.x);
  int y_res = max(r1.y, r2.y);
  int width_res = min(r1.x + r1.width, r2.x + r2.width) - x_res;
  int height_res = min(r1.y + r1.height, r2.y + r2.height) - y_res;

  // set
  res.x = x_res;
  res.y = y_res;
  if (width_res < 0 || height_res < 0) {
    res.width = 0;
    res.height = 0;
  }
  else {
    res.width = width_res;
    res.height = height_res;
  }
  //  res.width = max(0, width_res);
  //res.height = max(0, height_res);
